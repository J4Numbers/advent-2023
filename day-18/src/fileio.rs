...
pub(crate) fn read_file<'h, P: std::convert::AsRef<Path>>(file_loc: P) -> Vec<digger::DigInstruction> {
    let mut ret_lines: Vec<digger::DigInstruction> = vec![];
    let re = Regex::new(r"([UDLR]) ([0-9]+) \((#[0-9a-f]{6})\)").unwrap();
    if let Ok(lines) = read_lines(file_loc) {
        for test_line in lines {
            if  let Ok(line) = test_line {
                for (_, [dir, count, col]) in re.captures_iter(&*line).map(|c| c.extract()) {
                    ret_lines.push(digger::DigInstruction{ direction: dir.chars().nth(0).unwrap(), count: count.parse::<i32>().unwrap(), colour: String::from(col) })
                }
            }
        }
    }
